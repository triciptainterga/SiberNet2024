...
$(document).on("change", "input[name='attachmentforward']", function (e) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        var filename = this.files[0].name
        var fileextension = filename.split('.').pop();
        if (fileextension == "xls" || fileextension == "xlsx" || fileextension == "mp4" || fileextension == "Mp4" || fileextension == "Mp4" || fileextension == "mp3" || fileextension == "doc" || fileextension == "docx" || fileextension == "pdf" || fileextension == "png" || fileextension == "PNG" || fileextension == "jpg" || fileextension == "JPG" || fileextension == "jpeg" || fileextension == "gif" || fileextension == "GIF" || fileextension == "bmp" || fileextension == "BMP") {
...
function EmailConversationNew(RefID) {
    $("#Div_Conversation").show();
    var FileInboxHTML = "" + IPSERVER + "/FileEmail/Inbox"
    var FileOutboxHTML = "" + IPSERVER + "/FileEmail/Outbox"
    var filenameimage = "";
    var result = "";
    var result_in = ""
    var messageDiv = $('#Journeymailconversationnew');
    $.ajax({
        type: "POST",
        url: "asmx/TrmMailConversation.asmx/UIDESK_TrxEmailConversation",
        data: "{RefID: '" + ReplaceConversation(RefID) + "', UserName: '" + $("#hd_sessionLogin").val() + "', Action: 'SELECT'}",
            ...
            for (i = 0; i < json.length; i++) {
                //if (json[i].DIRECTION == "IN") {
                    ...
                    await $.ajax({
                        type: "POST",
                        url: "asmx/TrmMailConversation.asmx/UIDESK_TrxEmailAttachment",
                        data: "{RefID: '" + ReplaceConversation(RefID) + "', EmailID: '" + json[i].EMAIL_ID + "', Direction: 'ALL', UserName: '" + $("#hd_sessionLogin").val() + "'}",
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (data) {
                            var json = JSON.parse(data.d);
